...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm () {
      api.delete_zone(this.edited_item.id)
          .then(() => {
            this.closeDelete()
            this.update()
    ...
    save () {
      if (this.editedIndex === -1) {
        api.add_zone({name: this.edited_item.id, enabled: this.edited_item.enabled})
            .then(() => {
              this.close()
              this.update()
    ...
    update() {
      ...
      this.loading = true
      api.get_zones(this.search, (page-1) * itemsPerPage, itemsPerPage, ascending)
          .then(resp => {
            this.zones = resp.data.data.items ? resp.data.data.items : []
